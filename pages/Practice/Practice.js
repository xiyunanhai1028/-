...
Page({
  ...
  data: {
    ...
    PaperList: [],
    tabList: [] ,
    paperType: '',//1:公务员 2: 事业单位 3：银行考试 4：政法干警考试 5：教师资格证 6：教师招聘 7：选调研究生 8：公务员（省考）
    paperTypeArr:[
      {
        id:1,
        name:"公务员"
  ...
  goPage(event) {
    var that = this;
    wx.navigateTo({
      url: event.currentTarget.dataset.page + "?paperType=" + that.data.paperType 
    })
  },  
  /**
   * 切换
  ...
  qiehuan(courseKind) {
    ...
    http.postRequest('reply/getPaperList', {
      "paperType": courseKind,//1:公务员 2: 事业单位 3：银行考试 4：政法干警考试 5：教师资格证 6：教师招聘 7：选调研究生 8：公务员（省考）
      "publishId": "1"//1:历史真题2:作业3:历史回答
    }, function (data) {
      that.setData({
        PaperList: data.body
      })
